        ...
        <Card className="border-0 shadow-2xl bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl">
          ...
          <CardContent className="space-y-4 pb-8">
            ...
            <Dialog
              ...
              onOpenChange={(value) => {
                setOpen(value)
                if (!value) setModalError(null)
              }}
            >
              <DialogTrigger asChild>
                <Button className="w-full h-12 bg-white dark:bg-gray-800 text-gray-900 dark:text-white border-2 border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-200 shadow-sm hover:shadow-md cursor-pointer">
                  Nikka
                </Button>
              </DialogTrigger>
              <DialogContent className="sm:max-w-md border-none shadow-2xl p-8 bg-white dark:bg-gray-900">
                ...
                {modalError && (
                  <div className="mb-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
                    <p className="text-sm text-red-600 dark:text-red-400 text-center">{modalError}</p>
                  </div>
                )}
                <form onSubmit={handleEmailLogin} className="space-y-5">
                  <div className="space-y-1.5">
                    <Label htmlFor="email" className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground ml-1">
                      Email
                    </Label>
                    <Input
                      id="email"
                  ...
                  >
                    {modalLoading ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : 'Belépés'}
                  </Button>
                </form>
              </DialogContent>
            </Dialog>
